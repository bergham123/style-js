...
(() => {
  ...
  function openEvent(event = {}) {
    ...
    $('#eventForm').onsubmit = async e => {
      ...
      // جمع بيانات التنبيهات (الأرقام والإيميلات فقط)
      const notifyVia = [];
      if ($('#chkTelegram').checked && $('#valTelegram').value.trim()) notifyVia.push($('#valTelegram').value.trim());
      if ($('#chkWhatsapp').checked && $('#valWhatsapp').value.trim()) notifyVia.push($('#valWhatsapp').value.trim());
      if ($('#chkGmail').checked && $('#valGmail').value.trim()) notifyVia.push($('#valGmail').value.trim());
